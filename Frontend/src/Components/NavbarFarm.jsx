...
function NavbarFarm() {
    ...
    return (
        <nav className='h-[10vh] w-[100vw] sm:w-auto sticky top-0 bg-green-400 flex items-center justify-between z-10'>
            <div className='bg-white inline-block'>
                <NavLink to='/'>
                    <img src={Logo} alt="logo" className='w-24 h-[10vh]' srcset="" />
                </NavLink>
            </div>
            ...
            <div className={`mx-4 px-10 w-[90vw] z-auto absolute h-[100vh] sm:h-full md:w-[85vw] lg:w-[60vw] bg-green-400 text-white font-bold flex gap-4 sm:gap-0 flex-col transition-all sm:flex-row  top-0 sm:static sm:justify-between items-center ${open ? 'left-[-10%]' : 'left-[-110%]'} `}>
            ...
            <div className={`block sm:hidden cursor-pointer text-white font-bold ${open ? 'hidden' : 'block'}`} onClick={() => setOpen(!open)}>
                <RiMenu3Fill />
            </div>
        </nav>
    )
